...
function buildSystemPrompt(char) {
    ...
    prompt += `- 如果用户要求你发照片、自拍、现拍、给他看现在的样子，必须把照片作为单独消息段输出：[微信图片:画面提示词|可选说明]。画面提示词必须按你自己的角色卡写清楚“你本人”的外观、发型发色、眼睛、服饰、表情、姿势、光线、环境、手机照片质感，并明确“参考图用于保持同一张脸和辨识度”；不要只用文字假装“照片传过来了”。\n`;
