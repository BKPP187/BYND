...
(() => {
    ...
    const resourceNames = {
        'apps/monitor/pet-background-worker.js': '处理程序',
    ...
    async function readWeb(url, path) {
        url.searchParams.set('v', path.startsWith(assetRoot) ? '1.29.0-u2netp-v1' : '1.1.698');
    ...
    async function infer(input, progress) {
        ...
        const [program, runtime, wasm, model] = await Promise.all([
            readResource('apps/monitor/pet-background-worker.js'), readResource(assetRoot + 'ort.wasm.bundle.min.mjs'),
