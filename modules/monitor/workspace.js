...
(() => {
    ...
    function createWorkspace(appName) {
        ...
        function stage(char) {
            ...
            const hasPet = !!(material?.posterDataUrl || (bound?.id === char?.id && getActiveMonitorPetId()));
            const displayed = !!char && hasPet && isMonitorPetEnabled() && bound?.id === char.id;
            return `<section class="mh-stage mh-stage-pet" aria-label="桌宠预览">
                <div class="mh-stage-top">${chooseButton(char)}${badge(displayed ? '当前桌宠' : '形象预览', displayed ? 'is-on' : '')}</div>
                <div class="mh-scene"><span class="mh-scene-word" aria-hidden="true">little moments</span>${petVisual(char)}</div>
                <div class="mh-stage-bottom"><div><span>${hasPet ? '当前形态' : '角色头像预览'}</span><strong>${hasPet ? escape(window.ByndCharacterPet?.visual(char)?.label || '待机') : '制作一个专属小伙伴'}</strong></div>
                    <button type="button" class="mh-round-button" data-mh-action="studio" aria-label="制作或编辑桌宠" ${!char ? 'disabled' : ''}>${icon('ri-magic-line')}</button>
                </div>
            </section>`;
        }
        ...
        function homeView() {
            ...
            return `${heading('聊天，多一个视角。', '选择监控者，让角色参与剧情、发弹幕和提醒。')}
                ...
                <button type="button" class="mh-cross-app" data-mh-action="open-pet-screen">${icon('ri-screenshot-2-line')}<span><strong>屏幕陪看</strong><small>在桌宠中管理共享画面与角色互动</small></span>${icon('ri-arrow-right-s-line')}</button>`;
        }
        ...
        function petView() {
            ...
            return `${heading('小小一只，属于你。', '同一个角色，也有不同的小表情。')}
                <div class="mh-pet-layout"><div>${stage(char)}<div class="mh-pet-toggle-row"><span><strong>显示桌宠</strong><small>${bound ? `当前绑定 ${escape(name(bound))}` : '还没有绑定角色'}</small></span><button type="button" class="mh-switch" role="switch" aria-label="显示桌宠" aria-checked="${enabled}" data-mh-action="toggle-pet"><span></span></button></div>
                    ${char && bound?.id !== char.id ? `<button class="mh-bind-button" type="button" data-mh-action="bind-pet">绑定 ${escape(name(char))} 为桌宠${icon('ri-arrow-right-line')}</button>` : ''}</div>
        ...
        function render() {
            ...
            host.querySelector('.mh-overview').innerHTML = `<span class="mh-presence ${(isPet ? isMonitorPetEnabled() && getMonitorPetBoundChar() : stats.enabled) ? 'is-on' : ''}"><span aria-hidden="true"></span>${isPet ? (isMonitorPetEnabled() ? '已开启' : '已关闭') : `${stats.enabled} 位接入`}</span>`;
            ...
            panel.innerHTML = activeTab === 'phone' ? screenView() : activeTab === 'pet' ? petView() : activeTab === 'island' ? activityView() : homeView();
        ...
        function roleRows() {
            ...
            return chars.map(char => `<div class="mh-role-row ${character()?.id === char.id ? 'is-selected' : ''}"><button type="button" class="mh-role-choice" data-mh-role="${attr(char.id)}" aria-pressed="${character()?.id === char.id}">${avatar(char)}<span><strong>${escape(name(char))}</strong><small>${isPet ? (getMonitorPetBoundChar()?.id === char.id ? '当前绑定桌宠' : '选择查看或绑定桌宠') : (char.chatConfig?.monitorEnabled ? '已接入监控' : '待命')}</small></span>${character()?.id === char.id ? icon('ri-check-line') : ''}</button>${isPet ? '' : `<button type="button" class="mh-icon-button" data-mh-settings="${attr(char.id)}" aria-label="设置 ${attr(name(char))}">${icon('ri-equalizer-line')}</button>`}</div>`).join('') || '<p class="mh-quiet-empty">没有找到角色，换个名字试试。</p>';
        }
        ...
        function renderLayer() {
            ...
            let title = '', body = '';
            if (layer.type === 'roles') {
                title = isPet ? (layer.purpose === 'binding' ? '绑定桌宠角色' : '选择桌宠角色') : '监控角色设置';
                body = `<p class="mh-sheet-intro">${isPet ? '从已有角色中选择，使用这位角色的人设与桌宠素材。' : '点击角色或右侧设置，管理接入状态、视角和弹幕速度。'}</p><label class="mh-search">${icon('ri-search-line')}<input id="${appName}-role-search" type="search" aria-label="搜索角色" placeholder="搜索角色名字" value="${attr(roleQuery)}"></label><div class="mh-role-list">${getMonitorCharacters().length ? roleRows() : empty('还没有角色', '先到微信添加你喜欢的角色。')}</div><button type="button" class="mh-add-role" data-mh-action="contacts">${icon('ri-user-add-line')}前往微信添加角色${icon('ri-arrow-right-s-line')}</button>`;
            ...
            host.innerHTML = `<div class="mh-layer"><section class="mh-sheet" role="dialog" aria-modal="true" aria-labelledby="${appName}-sheet-title"><div class="mh-sheet-handle" aria-hidden="true"></div><header class="mh-sheet-heading">${layer.type === 'settings' ? '<button type="button" class="mh-icon-button" data-mh-action="roles" aria-label="返回角色列表">' + icon('ri-arrow-left-s-line') + '</button>' : ''}<h2 id="${appName}-sheet-title">${escape(title)}</h2><button type="button" id="${appName}-sheet-close" class="mh-icon-button" data-mh-action="close-layer" aria-label="关闭面板">${icon('ri-close-line')}</button></header><div class="mh-sheet-body">${body}</div></section></div>`;
        ...
        function navigate(tabId) {
            ...
            petPane = 'own';
            render();
            const content = root()?.querySelector('.mh-content'); if (content) content.scrollTop = 0;
        }
        const api = {
