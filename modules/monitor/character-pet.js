...
(() => {
    ...
    function assetInfo(key, value) {
        ...
        return {
            ...
            source: ['generated', 'generated-animation', 'background-removal', 'upload'].includes(value.source) ? value.source : value.prompt ? 'generated' : 'upload'
        };
    }
    ...
    function imagePrompt(char, state = null, removeBackground = false, animation = false) {
        ...
        return [
            animation ? '【动画序列任务】基于用户已确认的桌宠，为下方一种姿态制作连续轻缓的小动作。输出严格 2×2 等分网格的 4 帧透明 PNG 动画序列图，顺序左上→右上→左下→右下。每格重复同一角色或代表形态。四帧必须有可见但克制的动作进展，不能复制四张相同静帧；保持同一姿态的大方向，避免坐、站、躺之间瞬间跳变。不添加人设不允许的撒娇、爱心或亲昵动作。' : '',
            state ? '任务：为用户已确认的 Q 版 3D 桌宠制作一张动作与表情变体透明 PNG。围绕同一个角色或已确认代表形态构图。' : symbol ? '任务：把用户确认的角色代表元素制作成原创呆萌 Q 版 3D 桌宠透明 PNG。本次只绘制该代表形态，不额外添加人形角色；这张造型代表同一个角色。' : '任务：为桌面陪伴应用制作同一角色的原创呆萌 Q 版 3D 小玩偶透明 PNG。整体圆润、软糯、小小一只，具有安静发呆的松弛感（super-deformed chibi toy）。只生成单个完整主角色，可带有下方明确允许的陪伴物。',
            ...
            animation ? '【动画网格构图】整张画布 1024×1024，精确分成 4 个 512×512 方格。每格独立透明且四边至少 10% 空白，同一角色、必要标志物和接触物完整处于各自格内；不跨格、无格线、无数字、无背景。镜头、尺度、主体基准位置、服饰、脸部和物品在四帧间锁定。第一帧动作起点、第二帧自然过渡、第三帧轻微动作重点、第四帧回到接近起点，便于无突跳循环；全程保持所选坐、趴、躺等姿态。配饰、羽翼和接触物都不能裁切、消失、悬空或穿模。' : '【统一构图】1024×1024 正方形画布，主角色或代表形态与必要陪伴物组成一个完整小组合，四周约 10% 透明留白。按坐、趴、躺等姿态选择正面或轻微三分之四角度，能看清面部与动作；允许重心和组合宽高比变化，不拉伸回站姿。保留全部肢体、羽翼和物品，不裁切，不画多个角色或表情拼图。角色坐着、趴在或倚靠物品时接触关系清楚，物品大小协调，不能穿模或悬空。',
            '【输出】RGBA PNG，真实 Alpha 透明通道。无环境背景、无关地板、装饰性产品底座、外部地面阴影、背景光晕、棋盘格图案、文字和水印。用户指定或有人设依据的座椅、支撑物、标志物与陪伴物属于主体，必须保留；组合内部可有柔和立体明暗和必要接触阴影。轮廓完整，无白边灰边。'
        ].filter(Boolean).join('\n\n');
    }
