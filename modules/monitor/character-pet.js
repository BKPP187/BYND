...
(() => {
    ...
    function imagePrompt(char, state = null, removeBackground = false) {
        ...
        return [
            '任务：为桌面陪伴应用制作同一角色的 Q 版 3D 手办风格透明 PNG。只生成单个完整角色。',
            state ? '第一张图片是用户已确认的角色母版。基于该母版编辑，只调整下方指定的表情与动作。保持同一张脸、相同发型发色、瞳色、肤色、年龄气质、服装配饰、材质、灯光、镜头和角色比例。' : '第一张图片锁定角色身份。第二张图片仅参考大头小身比例、柔和体积感、细腻哑光手办材质、分束蓬松头发和柔光。不复制风格示例人物的白发、五官、灰色衣服或身份。',
