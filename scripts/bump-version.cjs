...
function currentVersion() {
    const match = read('settings.js').match(/const APP_VERSION = 'v(\d+\.\d+\.\d+)';/);
    if (!match) throw new Error('settings.js APP_VERSION not found');
    return match[1];
}
...
const RULES = [
    ...
    ['style.css', (text, v) => text.replace(/(@import url\(["']?\.\/[^"')?]+\.css)\?v=[^"')]+/g, `$1?v=${v}`)],
    ['manifest.webmanifest', (text, v) => text.replace(/("start_url": "\/\?v=)[^"]+"/, `$1${v}"`)],
    ['settings.js', (text, v) => text.replace(/const APP_VERSION = 'v\d+\.\d+\.\d+';/, `const APP_VERSION = 'v${v}';`)],
    ['script.js', (text, v) => text
    ...
    ['modules/monitor/pet-background.js', (text, v) => text.replace(/(path\.startsWith\(assetRoot\) \? '[^']+' : ')[^']+'/, `$1${v}'`)],
