...
assert.match(style, /\.bynd-startup\s*\{[^}]*display:\s*none;/s, 'fast loads must not display the loading overlay by default');
assert.match(style, /\.bynd-startup-loading \.bynd-startup\s*\{\s*display:\s*grid;/, 'only pending slow loads may display the overlay');
