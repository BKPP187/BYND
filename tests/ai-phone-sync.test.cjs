...
test('a full-phone update preserves cached letters without requesting a diary repair', async () => {
    const h = harness();
    h.char.chatConfig.aiPhoneSnapshot = savedPhone({ diaryLetters: letters() });
    const before = clone(h.char.chatConfig.aiPhoneSnapshot);
    h.state.respond = async () => h.state.requests.length === 1 ? phoneResponse({ diaryLetters: [] }) : ({ ok: false, error: 'API 错误 (429)' });
    const result = await h.context.requestWechatAiPhoneSnapshot(h.char, { force: true });
    assert.equal(h.state.requests.length, 1, 'saved letters must not be regenerated by a phone update');
    assert.deepEqual(clone(result.diaryLetters), before.diaryLetters);
    assert.equal(result.memos[0].title, '新的备忘录');
    assert.equal(result.diarySyncError, undefined);
    assert.deepEqual(h.state.savedChar.chatConfig.aiPhoneSnapshot.diaryLetters, before.diaryLetters);
});

test('new chat turns update other phone data while saved letters survive repeated opens and reloads', async () => {
    const h = harness();
    h.char.chatConfig.aiPhoneSnapshot = savedPhone({ diaryLetters: letters(), diaryUpdatedAt: now - 5000, syncTurnKey: 'previous-turn' });
    h.char.history = [message(true, now - 100), message(false, now - 50)];
    const before = clone(h.char.chatConfig.aiPhoneSnapshot);
    const pending = deferred();
    h.state.respond = () => pending.promise;
    h.open();
    assert.equal(h.context.window._wechatAiPhoneGenerating.has(h.char.id), true);
    assert.equal(!!h.context.window._wechatAiPhoneDiaryGenerating?.has(h.char.id), false);
    pending.resolve(phoneResponse({ diaryLetters: letters().map(letter => ({ ...letter, title: '不应替换的信件', body: letter.body.replace('河边', '公园') })) }));
    await h.finish();
    assert.deepEqual(clone(h.char.chatConfig.aiPhoneSnapshot.diaryLetters), before.diaryLetters);
...
test('phone storage failures leave the saved diary and its timestamp intact', async () => {
    for (const failureAt of [1, 2]) {
        const h = harness();
        h.char.chatConfig.aiPhoneSnapshot = savedPhone({ diaryLetters: letters(), diaryUpdatedAt: now - 5000 });
        const before = clone(h.char.chatConfig.aiPhoneSnapshot);
        h.char.history = [message(true, now - 100), message(false, now - 50)];
        h.state.saveResult = count => count !== failureAt;
        h.open();
...
test('explicit diary refresh shows its own pending state, saves new letters once and clears the state', async () => {
    const h = harness();
    h.char.chatConfig.aiPhoneSnapshot = savedPhone({ diaryLetters: letters(), diaryUpdatedAt: now - 5000 });
    const pending = deferred();
    h.state.respond = () => pending.promise;
    const first = h.context.regenerateWechatAiPhoneDiary(h.char.id);
    assert.equal(h.context.window._wechatAiPhoneDiaryGenerating.has(h.char.id), true);
    h.open();
    h.close();
    h.open();
    const second = h.context.regenerateWechatAiPhoneDiary(h.char.id);
    const nextLetters = letters().map(letter => ({ ...letter, title: '新的' + letter.title, body: letter.body.replace('河边', '公园') }));
    pending.resolve(phoneResponse({ diaryLetters: nextLetters }));
    await Promise.all([first, second]);
    assert.equal(h.state.requests.length, 1);
    assert.deepEqual(clone(h.char.chatConfig.aiPhoneSnapshot.diaryLetters), nextLetters);
    assert.equal(h.char.chatConfig.aiPhoneSnapshot.diaryUpdatedAt, now);
    assert.equal(h.context.window._wechatAiPhoneDiaryGenerating.size, 0);
    assert.deepEqual(h.state.savedChar.chatConfig.aiPhoneSnapshot.diaryLetters, nextLetters);
    const reloaded = harness(h.state.savedChar);
    reloaded.open();
    await reloaded.finish();
    assert.equal(reloaded.state.requests.length, 0);
