...
(function () {
    ...
    function dateBounds(from = '', to = '') {
        ...
        if (Number.isNaN(start) || Number.isNaN(endDay)) return { valid: false, error: '请选择有效日期。' };
        if (start != null && endDay != null && start > endDay) return { valid: false, error: '开始日期不能晚于结束日期。' };
        const endDate = endDay == null ? null : new Date(endDay);
        const end = endDate == null ? null : new Date(endDate.getFullYear(), endDate.getMonth(), endDate.getDate() + 1).getTime();
    ...
    async function buildViewModel(ledger, view = state, now = Date.now()) {
        ...
        const dates = dateBounds(view.dateFrom, view.dateTo);
        if (!dates.valid) throw new Error(dates.error);
        ...
        if (!view.path.length) {
            ...
            const endYear = dates.to == null ? Math.max(nowYear, dates.from == null ? nowYear : new Date(dates.from).getFullYear()) : new Date(dates.to - 1).getFullYear();
            const firstQueryYear = dates.from == null ? endYear - YEAR_LOOKBACK : new Date(dates.from).getFullYear();
            entries = await ledger.list({ ...filters, from: dates.from ?? new Date(firstQueryYear, 0, 1).getTime(), to: dates.to ?? new Date(endYear + 1, 0, 1).getTime() });
            const firstAt = (entries || []).map(entry => Number(entry?.at)).find(Number.isFinite);
            const firstYear = firstAt != null ? new Date(firstAt).getFullYear() : endYear;
            const startYear = dates.from == null ? Math.min(endYear - 2, Math.max(firstYear, endYear - 5)) : new Date(dates.from).getFullYear();
            range = buildRange([], { startYear, endYear });
        } else {
            range = buildRange(view.path);
            const from = Math.max(range.from, dates.from ?? range.from);
            const to = Math.min(range.to, dates.to ?? range.to);
            entries = to > from ? await ledger.list({ ...filters, from, to }) : [];
        }
    ...
    function renderHtml(model, view = state, options = {}) {
        ...
        const dateFilter = `<form class="bill-date-filter" aria-label="日期筛选"><div class="bill-date-fields"><label>开始日期<input type="date" name="from" value="${escapeHtml(view.dateFrom || '')}" aria-label="开始日期"></label><label>结束日期<input type="date" name="to" value="${escapeHtml(view.dateTo || '')}" aria-label="结束日期"></label></div><div class="bill-date-actions"><button type="submit">筛选日期</button><button type="button" data-action="clear-dates" ${view.dateFrom || view.dateTo ? '' : 'disabled'}>清除日期</button></div><span class="bill-date-error" role="alert" aria-live="polite"></span></form>`;
        ...
        return `${head}${units}${dateFilter}${trail}${renderFilters(model, view)}<div class="bill-rule"></div>${renderSummary(model)}<div class="bill-barcode" aria-hidden="true"></div>${emptyNote}${chart}${features}${requests}${renderPrices(model, view)}${end}`;
    }
    ...
    function bindEvents(root) {
        ...
        root.addEventListener('submit', async event => {
            const dateForm = event.target.closest?.('form.bill-date-filter');
            if (dateForm) {
                event.preventDefault();
                const from = dateForm.elements.from.value;
                const to = dateForm.elements.to.value;
                const dates = dateBounds(from, to);
                if (!dates.valid) {
                    dateForm.querySelector('.bill-date-error').textContent = dates.error;
                    return;
                }
                state.dateFrom = from;
                state.dateTo = to;
    ...
    window.ByndBillApp = {
        ...
        render,
        _internal: {
            UNITS, METRICS, buildRange, bucketIndexFor, pathFromTime, crumbsFor, drillPath, crumbPath, unitPath, dateBounds,
